...
import tensorboard
# tensorboard --logdir=results/sb3/hidden/A2C_2
...
class CustomCNN(torch_layers.BaseFeaturesExtractor):
  ...
  def __init__(
      ...
      features_dim=128,
      num_frames=6,
      fcnet_hiddens=(1024, 128), #(1024,128)
  ):
...
def main():
  # Config
  env_name = "hidden_agenda"
  env_config = substrate.get_config(env_name)
  env = utils.parallel_env(env_config)
  rollout_len = 1000 # 更改 1000
  total_timesteps = int(1e9) #100M
  save_interval = int(1e6) #1M
  num_agents = env.max_num_agents

  # Training
  num_cpus = 20  # number of cpus
  num_envs = 20  # number of parallel multi-agent environments
  ...
  grad_clip = 40
  verbose = 3
  model_path = "./results/sb3/hidden/A2C_2/model169000000.zip"  # Replace this with a saved model
  ...
  eval_freq = save_interval//(num_envs * num_agents ) # eval_freq*num_envs * num_agents= 真eval间隔
  ...
  tensorboard_log = "./results/sb3/hidden"
  ...
  if model_path is not None:
    model = stable_baselines3.A2C.load(model_path, env=env)
  eval_callback = callbacks.EvalCallback(
      eval_env, eval_freq=eval_freq, best_model_save_path=tensorboard_log)

  model.learn(total_timesteps=save_interval, reset_num_timesteps=(model_path is None), callback=eval_callback)
  logdir = model.logger.dir
  model.save(logdir + f"/model{save_interval}")
  for i in range(total_timesteps//save_interval-1):
    model.learn(total_timesteps=save_interval,reset_num_timesteps=False, callback=eval_callback)
    model.save(logdir + f"/model{model.num_timesteps }") #这里应该获取 真step


  del model
  stable_baselines3.A2C.load(logdir + f"/model{total_timesteps}")
