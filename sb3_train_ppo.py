...
import tensorboard
# tensorboard --logdir=results/sb3/harvest_open_a2c_paramsharing/A2C_21
...
def main():
  ...
  rollout_len = 8 # 更改 1000
  total_steps = 150 # 真step=total_timesteps * rollout_len
  save_interval = 10 #
  ...
  eval_freq = save_interval*rollout_len//(num_envs * num_agents) # eval_freq*num_envs * num_agents= 真eval间隔
  ...
  model = stable_baselines3.PPO(
      ...
      n_epochs=n_epochs, # Number of epoch when optimizing the surrogate loss ?
      gamma=gamma,
  ...
  # model.save(logdir + f"/model{save_interval*rollout_len*(num_envs * num_agents )}")
  #for i in range(120//30-1):
  model.learn(total_timesteps=20,reset_num_timesteps=False) #如果reset_num_timesteps=False， 每step这个是固定的 = roll*env*agent
  model.learn(total_timesteps=30,reset_num_timesteps=False)
  model.learn(total_timesteps=30,reset_num_timesteps=False)
  #logdir = model.logger.dir
  #model.save(logdir + f"/model{save_interval*(i+2)*rollout_len*(num_envs * num_agents )}")
